...
**인증 관련 (4개)**
- `GET /api/auth/kakao/login` - 카카오 로그인 URL 리다이렉트
- `GET /api/auth/kakao/callback` - 카카오 로그인 콜백 처리 및 JWT 발급
- `GET /api/auth/me` - 현재 로그인된 사용자 정보 조회 (JWT 인증 필요)
- `POST /api/auth/logout` - 로그아웃 처리

**사용자 관리 (9개)**
- `GET /api/user/profile` - 사용자 프로필 정보 조회
- `PUT /api/user/profile` - 사용자 프로필 정보 수정 (닉네임, 프로필 이미지 등)
- `GET /api/user/categories` - 사용자 관심 카테고리 목록 조회
- `PUT /api/user/categories` - 사용자 관심 카테고리 목록 수정
- `GET /api/user/bookmarks` - 사용자 북마크한 뉴스 목록 조회
- `GET /api/user/frequencies` - 사용자 관심 카테고리별 오늘 주파수 조회
- `POST /api/user/onboarding` - 온보딩 완료 처리
- `GET /api/user/onboarding/status` - 온보딩 완료 여부 확인
- `GET /api/user/onboarding` - 온보딩 페이지 정보 제공

**카테고리 관리 (1개)**
- `GET /api/categories` - 전체 카테고리 목록 조회 (온보딩/설정 페이지용)

**뉴스 관련 (5개)**
- `GET /api/news` - 카테고리별 뉴스 목록 조회 (최대 10개, query parameter: category)
- `GET /api/news/today` - 오늘의 뉴스 카테고리별 그룹핑 (각 카테고리 6개씩)
- `GET /api/news/{news_id}` - 개별 뉴스 상세 내용 조회
- `POST /api/news/bookmark` - 뉴스 북마크 추가 (JWT 인증 필요)
- `DELETE /api/news/bookmark/{news_id}` - 뉴스 북마크 삭제 (JWT 인증 필요)

**주파수(팟캐스트) 관련 (3개)**
- `GET /api/frequencies` - 사용자 관심 카테고리별 오늘 주파수 목록
- `GET /api/frequencies/history` - 사용자 관심 카테고리별 주파수 히스토리 (과거 데이터)
- `GET /api/frequencies/{category}` - 특정 카테고리 주파수 상세 정보 조회
...
**오디오 스트리밍**
- ElevenLabs 고품질 TTS
- S3 Presigned URL (7일 유효, 요청했는데 기간 만료시 재생성)
...
- 사용자 인증 → 카테고리 설정 → 오늘의 뉴스 & 내 주파수 서비스
- 에러 상황 대응 및 복구 프로세스
