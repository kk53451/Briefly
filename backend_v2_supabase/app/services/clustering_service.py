...
# 광고/연성 콘텐츠(스폰서드) 휴리스틱
# ──────────────────────────────────────────────
# 제목에 이런 brackets 가 들어있는 기사는 같은 시리즈로 매일 반복 게시되어
# 거대 클러스터를 형성하는 경향이 있다. 토픽 랭킹에서 제외.
ADVERTORIAL_TITLE_PATTERNS: List[re.Pattern] = [
    re.compile(r"\[헬시타임\]"),
    re.compile(r"\[건강\+?\]"),
    re.compile(r"\[리뷰\]"),
    re.compile(r"\[광고\]"),
...
# 멤버 중 광고성 제목 비율이 이 값 이상이면 광고 클러스터로 판정
ADVERTORIAL_TITLE_RATIO = 0.40

# 단일 언론사 검사: 멤버 N건 이상일 때만 의미 있게 적용
ADVERTORIAL_MIN_SIZE_FOR_PRESS_CHECK = 25

# 한 언론사가 클러스터의 이 비율 이상 점유하면 광고/시리즈 의심
ADVERTORIAL_SINGLE_PRESS_RATIO = 0.55
...
def _tokenize_text(text: str) -> List[str]:
    ...
    # 한글 복합어 정규식 보강은 Okt 가 정상 동작하면 어절(조사·어미 포함)을
    # 통째로 잡아 키워드를 오염시킨다 — 예: '국민의힘에서', '기자회견에서',
    # '평가받겠다', '대표는'. Okt 가 명사를 정확히 분리해 주므로 정규식은
    # KoNLPy 로드 실패 시 fallback 으로만 사용한다.
    if okt is None:
...
def _merge_overlapping_keywords(counter: Counter) -> Counter:
    """짧은 토큰이 더 긴 토큰의 substring 이면 score 를 긴 쪽으로 합산하고 짧은 쪽을 제거.

    Okt 가 같은 회사·인명을 다르게 쪼개 동일 의미가 top 슬롯을 중복 점유하는 문제 완화.
    예: ('SK', 5), ('하이닉스', 8), ('SK하이닉스', 15)  →  ('SK하이닉스', 28)
        ('삼성', 4), ('전자', 3), ('삼성전자', 9)        →  ('삼성전자', 16)

    한국어 substring 합치기는 의미 충돌 위험이 있지만(예: '한국' ⊂ '한국은행'),
    토픽 키워드 맥락에서는 같은 클러스터 안에서 함께 등장한 토큰이라 같은 개체일
    가능성이 높아 합치는 쪽이 신호 대비 잡음을 줄인다.
    """
    if not counter:
        return counter

    items = sorted(counter.items(), key=lambda x: (-len(x[0]), -x[1]))
    kept: List[List] = []  # [[tok, count], ...] — 가변 list 로 in-place 합산
    for tok, count in items:
        absorbed = False
        for entry in kept:
            longer_tok = entry[0]
            if tok != longer_tok and tok in longer_tok:
                entry[1] += count
                absorbed = True
                break
        if not absorbed:
...
def extract_topic_keywords(
    ...
    Okt 가 'SK / 하이닉스 / SK하이닉스' 처럼 같은 의미를 여러 토큰으로 쪼개는
    경우가 많아, 마지막에 _merge_overlapping_keywords 로 substring 흡수 처리.
    ...
    counter = _merge_overlapping_keywords(counter)
    return counter.most_common(top_k)


def detect_advertorial_cluster(
    cluster_members: List[int],
    articles: List[Dict],
) -> Tuple[bool, str]:
    """광고/연성 콘텐츠(스폰서드 시리즈) 클러스터인지 휴리스틱 판정.

    신호 두 가지:
      1. 멤버 중 ADVERTORIAL_TITLE_RATIO 이상이 광고성 brackets 제목 ([헬시타임] 등)
      2. 단일 언론사가 클러스터를 ADVERTORIAL_SINGLE_PRESS_RATIO 이상 점유
         (멤버 ADVERTORIAL_MIN_SIZE_FOR_PRESS_CHECK 이상일 때만 적용 — 작은
         클러스터는 단일 언론사 점유가 자연스러움)

    Returns:
        (is_advertorial, reason)  — reason 은 로그용 문자열
    """
    members = [articles[i] for i in cluster_members if i < len(articles)]
    if not members:
        return False, ""
    ...
    for art in members:
        title = art.get("title", "") or ""
        if any(p.search(title) for p in ADVERTORIAL_TITLE_PATTERNS):
            ad_title_count += 1

    if ad_title_count and ad_title_count / len(members) >= ADVERTORIAL_TITLE_RATIO:
        return True, f"advertorial titles {ad_title_count}/{len(members)}"

    # 신호 2: 단일 언론사 지배
    if len(members) >= ADVERTORIAL_MIN_SIZE_FOR_PRESS_CHECK:
        presses = [_get_article_press(a) for a in members]
        presses = [p for p in presses if p]
        if presses:
            top_press, top_count = Counter(presses).most_common(1)[0]
            ratio = top_count / len(presses)
            if ratio >= ADVERTORIAL_SINGLE_PRESS_RATIO:
                return True, f"single-press dominance: {top_press} {ratio:.0%}"
...
def _run_umap_hdbscan(
    ...
    # UMAP
    # 작은 n_samples 에서 두 가지가 동시에 터짐:
    #   1) n_neighbors >= n_samples 면 UMAP 자체가 거부
    #   2) spectral_layout 이 scipy.eigsh 에서 sparse k>=N 으로 죽음
    #      (scipy 1.13+ 에서 더 엄격해짐). init="random" 으로 우회.
    n_samples = len(embeddings)
    safe_neighbors = max(2, min(15, n_samples - 1))
    init_method = "spectral" if n_samples > 30 else "random"
...
def rank_topics(
    ...
    **Phase 2: 가중 점수 기반 랭킹**
    ...
    for cid, members in clusters.items():
        # 광고/스폰서드 시리즈 클러스터는 점수 계산을 건너뛰고 -1.0 패널티를
        # 부여해 top_n 정렬에서 사실상 제외. 점수 자체는 score_detail 에 남겨
        # 디버깅 시 어떤 사유로 빠졌는지 추적 가능하게 한다.
        is_ad, ad_reason = detect_advertorial_cluster(members, articles)
        if is_ad:
            ...
            score_detail = {
                ...
                "total_score": -1.0,
                "keywords": [],
                "advertorial_reason": ad_reason,
            }
            scored.append((-1.0, cid, members, score_detail))
            continue
    ...
    if advertorial_skipped:
        logger.info(
            f"  🚫 광고/스폰서드 클러스터 {advertorial_skipped}개 패널티 적용 (top_n 제외)"
        )
    ...
    for rank, (total_score, cid, members, score_detail) in enumerate(selected):
        ...
        # 대표 기사 제목에 광고성 brackets 가 박혀 있으면 토픽 자체를 제외.
        # 첫 번째 루프의 detect_advertorial_cluster 는 멤버 비율 기반이라
        # 시리즈 멤버가 1~2건만 묶인 거대 클러스터(예: 헬시타임 + 일반 건강 기사들)는
        # 통과시킨다. 대표가 시리즈 글이면 사용자에게 보여서는 안 되므로 추가 차단.
        rep_title = articles[rep_idx].get("title", "") or ""
        if any(p.search(rep_title) for p in ADVERTORIAL_TITLE_PATTERNS):
            logger.info(
                f"  🚫 광고 대표기사 토픽 제외 (size={len(members)}): {rep_title[:60]}"
            )
            continue
